...
nisbn.setBounds(25,20,100,20);
titre=new JLabel();
book.add(titre);
titre.setText("New Title");
titre.setFont(new Font("serif",Font.PLAIN,20));
titre.setBounds(25,80,100,20);

auteur=new JLabel();
book.add(auteur);
auteur.setText("New Author");
auteur.setFont(new Font("serif",Font.PLAIN,20));
auteur.setBounds(25,140,100,20);
//textfield
tnisbn=new JTextField();
book.add(tnisbn);
tnisbn.setBounds(145,20,100,20);
ttitre=new JTextField();
book.add(ttitre);
ttitre.setBounds(145,80,100,20);
tauteur=new JTextField();
book.add(tauteur);
tauteur.setBounds(145,140,100,20);
//boutton
		...
		if(e.getSource()==ok) {
			if(tnisbn.equals("")||ttitre.equals("")||tauteur.equals("")) {
				JOptionPane.showMessageDialog(null,"remplissez les zones de textes ","",JOptionPane.INFORMATION_MESSAGE);
		         ...
		         int x=database.livre_database.update_data(tnisbn.getText(),ttitre.getText(),tauteur.getText());
		         if(x==1) {
		                   JOptionPane.showMessageDialog(null,"ok","update",JOptionPane.INFORMATION_MESSAGE);
		                    tnisbn.setText("");
		                    ttitre.setText("");
		                    tauteur.setText("");
		                    this.dispose();
		                	...
		                	JOptionPane.showMessageDialog(null,"usager n'exist pas","update",JOptionPane.WARNING_MESSAGE);
		                	tnisbn.setText("");
		                    ttitre.setText("");
		                    tauteur.setText("");
		                     }
